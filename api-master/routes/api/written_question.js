...
router.get('/', middleware.checkToken,(req, res) => {
    ...
    aggregate.sort({"question" : 1}).match({"status":"active"})
...
router.get('/students/exam',middleware.checkToken, (req, res,next) => {

  Question.findRandom({"status":"active"}, {}, {limit: 1},function (err,element){
            if(err)
            {
               console.log("error")
            }
            ...
            {
                res.send({msg:"Questionss",status:true,description:"details found",element}) 
            }
